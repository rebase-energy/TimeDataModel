...
-------------
>>> import pandas as pd
>>> from timedb.timeseries import TimeSeries, DataType, Location
>>>
>>> df = pd.DataFrame({
...
...     location=Location(longitude=18.07, latitude=59.33),
... )
>>> print(ts)
...
@dataclass
class Location:
    """Minimal geographic location attached to a series."""
...
class DataType(str, Enum):
    """Semantic nature of the observations in the series."""
...
class TimeSeriesType(str, Enum):
    """Storage/versioning model for the series, mirroring the ``overlapping``
    flag in ``series_table``."""

    FLAT        = "flat"        # immutable facts
    OVERLAPPING = "overlapping" # versioned estimates / forecasts
...
class TimeSeries:
    ...
    location:
        Optional geographic location (:class:`Location`).
    timeseries_type:
    ...
    def from_pandas(
        ...
        * ``AUDIT``     — if ``change_time`` is present.
        * ``VERSIONED`` — if ``knowledge_time`` is present (but not
          ``change_time``).
        * ``SIMPLE``    — otherwise.
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"TimeSeries({name_str}, "
            f"shape={self._shape.value}, "
            f"rows={self.num_rows}, "
            f"unit='{self.unit}', "
            f"type={self.timeseries_type.value})"
        )
