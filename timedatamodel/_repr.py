...
# ---------------------------------------------------------------------------
# Mixin: _TimeSeriesNumpyReprMixin
# ---------------------------------------------------------------------------


class _TimeSeriesNumpyReprMixin:
    """Repr mixin for the NumPy-backed TimeSeriesNumpy class (timeseries_numpy.py)."""
    ...
    def _repr_data_rows(self, indices: list[int]) -> list[list[str]]:
        """Return one [timestamp_str, value_str] row per index, reading from NumPy arrays."""
        import pandas as pd
        ...
        for i in indices:
            ts_parts = [
                _fmt_short_date(pd.Timestamp(self._data[col][i]).to_pydatetime())
                for col in idx_cols
            ]
            val = self._data["value"][i]
            rows.append([", ".join(ts_parts), _fmt_value(None if (isinstance(val, float) and val != val) else val)])
        return rows
    ...
    def __repr__(self) -> str:
        ...
        return _render_box("TimeSeriesNumpy", content_lines)
    ...
    def _repr_html_(self) -> str:
        ...
        def _html_row(i: int) -> str:
            idx_cells = "".join(
                f"<td>{escape(_fmt_short_date(pd.Timestamp(self._data[col][i]).to_pydatetime()))}</td>"
                for col in idx_cols
            )
            val = self._data["value"][i]
            py_val = None if (isinstance(val, float) and val != val) else val
            return f"<tr>{idx_cells}<td>{escape(_fmt_value(py_val))}</td></tr>"

        return _build_repr_html(
            class_name="TimeSeriesNumpy",
            meta_rows=self._repr_meta_pairs(),
            index_names=idx_cols,
            column_names=col_names,
            n_rows=self.num_rows,
