...
from timedatamodel.coverage import CoverageBar
...
def cube_2d(timestamps_5h):
    ...
    dims = [
        Dimension("scenario", scenarios),
        Dimension("valid_time", timestamps_5h),
    ]
...
class TestConstruction:
    def test_2d_array(self, timestamps_5h):
        data = np.ones((3, 5))
        dims = [Dimension("scenario", ["a", "b", "c"]), Dimension("valid_time", timestamps_5h)]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
        assert cube.shape == (3, 5)

    def test_3d_array(self, timestamps_5h):
        data = np.ones((2, 5, 4))
        dims = [
            Dimension("scenario", ["a", "b"]),
            Dimension("valid_time", timestamps_5h),
            Dimension("quantile", [0.1, 0.25, 0.5, 0.9]),
        ]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
    ...
    def test_4d_array(self):
        ...
        dims = [
            Dimension("scenario", ["a", "b"]),
            Dimension("valid_time", ts),
            Dimension("quantile", [0.1, 0.25, 0.5, 0.9]),
            Dimension("model", ["m1", "m2"]),
        ]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
        assert cube.ndim == 4
    ...
    def test_plain_ndarray_wraps_nan(self, timestamps_5h):
        ...
        dims = [Dimension("scenario", ["a"]), Dimension("valid_time", timestamps_5h)]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
    ...
    def test_masked_array_preserved(self, timestamps_5h):
        ...
        dims = [Dimension("scenario", ["a", "b"]), Dimension("valid_time", timestamps_5h)]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
    ...
    def test_shape_mismatch_raises(self, timestamps_5h):
        ...
        with pytest.raises(ValueError, match="does not match"):
            TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
...
class TestProperties:
    ...
    def test_primary_time_dim_datetime_fallback(self, timestamps_5h):
        dims = [Dimension("time_axis", timestamps_5h), Dimension("x", ["a", "b"])]
        data = np.ones((5, 2))
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
        assert cube.primary_time_dim.name == "time_axis"

    def test_primary_time_dim_first_fallback(self):
        dims = [Dimension("x", ["a", "b"]), Dimension("y", ["c", "d"])]
        data = np.ones((2, 2))
        cube = TimeSeriesCube(Frequency.NONE, dimensions=dims, values=data)
...
class TestSel:
    ...
    def test_slice_keeps_axis(self, cube_2d, timestamps_5h):
        ...
        assert isinstance(result, TimeSeriesTable)
        assert len(result) == 3
    ...
    def test_sel_3d_keeps_cube(self, cube_3d, timestamps_5h):
        ...
        assert isinstance(result, TimeSeriesCube)
...
class TestFromTimeseriesList:
    def test_aligned_timestamps(self, timestamps_5h):
        s1 = TimeSeries(Frequency.PT1H, timestamps=timestamps_5h, values=[1.0]*5, name="a")
        s2 = TimeSeries(Frequency.PT1H, timestamps=timestamps_5h, values=[2.0]*5, name="b")
        dim = Dimension("scenario", ["s1", "s2"])
        cube = TimeSeriesCube.from_timeseries_list([s1, s2], dim)
    ...
    def test_ragged_timestamps(self):
        ...
        s1 = TimeSeries(Frequency.PT1H, timestamps=ts1, values=[1.0, 2.0, 3.0])
        s2 = TimeSeries(Frequency.PT1H, timestamps=ts2, values=[10.0, 20.0, 30.0])
        dim = Dimension("scenario", ["a", "b"])
        cube = TimeSeriesCube.from_timeseries_list([s1, s2], dim)
    ...
    def test_label_count_mismatch(self, timestamps_5h):
        s1 = TimeSeries(Frequency.PT1H, timestamps=timestamps_5h, values=[1.0]*5)
        dim = Dimension("scenario", ["a", "b"])  # 2 labels but 1 series
        with pytest.raises(ValueError, match="labels"):
            TimeSeriesCube.from_timeseries_list([s1], dim)

    def test_empty_list_raises(self):
        dim = Dimension("scenario", [])
        with pytest.raises(ValueError, match="empty"):
            TimeSeriesCube.from_timeseries_list([], dim)
...
class TestCoverageBar:
    def test_returns_coverage_bar(self, cube_2d):
        ...
        assert isinstance(cb, CoverageBar)
    ...
    def test_1d_cube_coverage(self, timestamps_5h):
        ...
        dims = [Dimension("valid_time", timestamps_5h)]
        cube = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
        cb = cube.coverage_bar()
        assert isinstance(cb, CoverageBar)
...
class TestNDTimeSeriesAlias:
    def test_alias_is_same_class(self):
        assert NDTimeSeries is TimeSeriesCube
...
class TestEquality:
    def test_equal_cubes(self, timestamps_5h):
        data = np.ones((2, 5))
        dims = [Dimension("scenario", ["a", "b"]), Dimension("valid_time", timestamps_5h)]
        c1 = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data)
        c2 = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=data.copy())
        assert c1.equals(c2)

    def test_unequal_cubes(self, timestamps_5h):
        dims = [Dimension("scenario", ["a", "b"]), Dimension("valid_time", timestamps_5h)]
        c1 = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=np.ones((2, 5)))
        c2 = TimeSeriesCube(Frequency.PT1H, dimensions=dims, values=np.zeros((2, 5)))
        assert not c1.equals(c2)

    def test_hash_is_none(self):
        assert TimeSeriesCube.__hash__ is None
