...
class TestGeoLocationDistance:
    ...
    def test_antipodal(self):
        ...
        b = GeoLocation(latitude=0, longitude=180)
...
class TestGeoLocationBearing:
    def test_due_north(self):
        a = GeoLocation(latitude=0, longitude=0)
        b = GeoLocation(latitude=10, longitude=0)
        assert abs(a.bearing_to(b) - 0.0) < 0.1

    def test_due_east(self):
        a = GeoLocation(latitude=0, longitude=0)
        b = GeoLocation(latitude=0, longitude=10)
        assert abs(a.bearing_to(b) - 90.0) < 0.1

    def test_due_south(self):
        a = GeoLocation(latitude=10, longitude=0)
        b = GeoLocation(latitude=0, longitude=0)
...
class TestGeoLocationMidpoint:
    ...
    def test_same_point(self):
        loc = GeoLocation(latitude=45, longitude=90)
...
class TestGeoLocationOffset:
    def test_round_trip(self):
        start = GeoLocation(latitude=60.0, longitude=10.0)
        bearing = start.bearing_to(GeoLocation(latitude=61.0, longitude=10.0))
...
class TestGeoAreaDistance:
    ...
    def test_outside_point(self):
        ...
        loc = GeoLocation(latitude=70.0, longitude=10.0)
        assert area.distance_to(loc) > 0.0

    def test_overlapping_areas(self):
        a = GeoArea.from_coordinates([
            (59.0, 9.0), (59.0, 12.0), (61.0, 12.0), (61.0, 9.0),
        ])
        b = GeoArea.from_coordinates([
...
class TestGeoAreaBoundingBox:
    def test_creates_valid_area(self):
        center = GeoLocation(latitude=60.0, longitude=10.0)
        bbox = GeoArea.bounding_box(center, 50.0, name="box")
    ...
    def test_approximate_size(self):
        center = GeoLocation(latitude=60.0, longitude=10.0)
        bbox = GeoArea.bounding_box(center, 50.0)
...
# ---- TimeSeriesTable spatial filtering ------------------------------------
...
def geo_table():
    oslo = GeoLocation(latitude=59.91, longitude=10.75)
    bergen = GeoLocation(latitude=60.39, longitude=5.32)
    trondheim = GeoLocation(latitude=63.43, longitude=10.40)
    timestamps = [datetime(2024, 1, 1), datetime(2024, 1, 2)]
    return TimeSeriesTable(
        Frequency.P1D,
        timestamps=timestamps,
        values=[[1, 2, 3], [4, 5, 6]],
        names=["Oslo", "Bergen", "Trondheim"],
        locations=[oslo, bergen, trondheim],
    )


class TestTableFilterByLocation:
    def test_filter_near_oslo(self, geo_table):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_table.filter_columns_by_location(oslo, 50.0)
        assert result.n_columns == 1
        assert result.column_names == ("Oslo",)

    def test_filter_wide_radius(self, geo_table):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_table.filter_columns_by_location(oslo, 500.0)
        assert result.n_columns >= 2

    def test_filter_no_match(self, geo_table):
        far = GeoLocation(latitude=0.0, longitude=0.0)
        result = geo_table.filter_columns_by_location(far, 10.0)
        assert result.n_columns == 0


class TestTableFilterByArea:
    def test_filter_area(self, geo_table):
        area = GeoArea.from_coordinates([
            (59.0, 10.0), (59.0, 11.0), (60.5, 11.0), (60.5, 10.0),
        ])
        result = geo_table.filter_columns_by_area(area)
        assert result.n_columns == 1
        assert result.column_names == ("Oslo",)


class TestTableNearest:
    def test_nearest_one(self, geo_table):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
...
# ---- TimeSeriesCollection spatial filtering --------------------------------
...
def geo_collection():
    ...
    return TimeSeriesCollection([ts_oslo, ts_bergen, ts_trondheim])


class TestCollectionFilterByLocation:
    def test_filter_near_oslo(self, geo_collection):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_collection.filter_by_location(oslo, 50.0)
        assert len(result) == 1
        assert "Oslo" in result

    def test_filter_wide_radius(self, geo_collection):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_collection.filter_by_location(oslo, 500.0)
        assert len(result) >= 2


class TestCollectionFilterByArea:
    def test_filter_area(self, geo_collection):
        area = GeoArea.from_coordinates([
            (59.0, 10.0), (59.0, 11.0), (60.5, 11.0), (60.5, 10.0),
        ])
        result = geo_collection.filter_by_area(area)
        assert len(result) == 1
        assert "Oslo" in result


class TestCollectionNearest:
    def test_nearest_one(self, geo_collection):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_collection.nearest(oslo, n=1)
        assert len(result) == 1
        assert "Oslo" in result

    def test_nearest_two(self, geo_collection):
        oslo = GeoLocation(latitude=59.91, longitude=10.75)
        result = geo_collection.nearest(oslo, n=2)
        assert len(result) == 2
